...
Call GetNewConnection2
   ...
   If cmbCust.text <> "Cash" Then
    ...
    Set Rs1 = New Recordset
      ...
      SQL = "update SalesRegistryDetail set Quantity=" & Val(.ListItems(CNT1).SubItems(2)) _
                    & ",Rate=" & Val(.ListItems(4).text) & " WHERE ProductID='" & .ListItems(CNT1).text & "'"

       DCON.Execute SQL
        
        SQL = "Delete * from SalesRegistryDetail where (PRODUCTID='" & .ListItems(CNT1).text & "' AND SalesRegistryID='" & Combo5.text & "') AND QUanity<=0"
